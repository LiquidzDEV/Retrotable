...
namespace LattePanda.Firmata
{
    ...
    class Arduino
    {
        ...
        public void PinMode(PinMapping pin, byte mode)
        {
            byte[] message = new byte[3];
            message[0] = (byte)(SET_PIN_MODE);
            message[1] = (byte)((int)pin);
            message[2] = (byte)(mode);
        ...
        public void DigitalWrite(PinMapping pin, byte value)
        {
            int portNumber = ((int)pin >> 3) & 0x0F;
            byte[] message = new byte[3];

            if ((int)value == 0)
                _digitalOutputData[portNumber] &= ~(1 << ((int)pin & 0x07));
            else
                _digitalOutputData[portNumber] |= (1 << ((int)pin & 0x07));
